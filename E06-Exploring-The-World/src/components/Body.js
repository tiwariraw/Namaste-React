...
const Body = () => {
    ...
    const fetchData = async () => {
        ...
        // console.log(arr1);    ---> array of restaurant objects
    ...
    const handleTopRatedRestaurants = () => {
        ...
        setRestaurantsArray(filteredRestaurants);
    }
    ...
    return restaurantsArray?.length === 0 ? <Shimmer /> : (
        <div className='body'>
            ...
            <div className='restaurant-cards-container'>
                {
                    filteredRestaurantsArray?.length === 0 ? <div className="no-restaurant-found">Sorry, we couldn't find any results for "<b>{searchText}</b>"</div> : filteredRestaurantsArray?.map(restaurant => (<RestaurantCard restaurantData={restaurant} key={restaurant?.info?.id} />))
                }
            </div>
        </div>
    )
